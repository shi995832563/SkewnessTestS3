...
# Plot
# ==========================================
setwd("C:/Users/shi99/OneDrive - The Chinese University of Hong Kong/New folder/Documents/Collaborator/sanying feng/revision")

postscript("simulated_meta_test.eps",horizontal = FALSE,width = 10,height = 6)

#par(mfrow = c(2, 2),mar = c(4, 3.5, 1.5, 1),oma = c(0, 0, 0, 0),mgp = c(2.5, 0.8, 0))

par(mfrow = c(2, 2), mar = c(4, 3.5, 1.5, 1), oma = c(0, 0, 0, 0), mgp = c(2.5, 0.8, 0)) 

cols <- c("purple", "darkgreen", "red", "black")
pchs <- c(15, 18, 16, 3)
ltys <- c(1, 1, 1, 2)

plot_panel <- function(y_mat, ylab, xlab = "", ref_line = NULL) {
  ylim <- range(y_mat, ref_line, na.rm = TRUE)
  pad <- 0.08 * diff(ylim)
  ...
  if (!is.null(ref_line)) {
    abline(h = ref_line, col = "gray50", lty = 2)
  }
}

plot_panel(bias_mat, ylab = "Bias", xlab = "", ref_line = 0)
plot_panel(mse_mat, ylab = "MSE", xlab = "")
...
plot_panel(cp_mat, ylab = "Coverage probability", xlab = expression(n), ref_line = 0.95)
plot_panel(ll_mat, ylab = "Length of 95% CI", xlab = expression(n))
